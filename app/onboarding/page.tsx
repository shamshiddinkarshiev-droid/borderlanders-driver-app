...
export default function OnboardingPage() {
  ...
  const getUrl = (res: any) => res?.[0]?.ufsUrl || res?.[0]?.url || res?.[0]?.fileUrl || '';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const urls = {
        ssn: getUrl(ssnRes), license: getUrl(licenseRes), registration: getUrl(regRes),
        insurance: getUrl(insRes), check: getUrl(checkRes),
        front: getUrl(frontRes), driverSide: getUrl(driverRes),
        passengerSide: getUrl(passengerRes), rear: getUrl(rearRes),
      };
