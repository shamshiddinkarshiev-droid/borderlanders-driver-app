...
export default function OnboardingPage() {
  ...
  const setPhoto = (type: PhotoType, file: File | null) => {
    ...
    if (file.size > MAX_FILE_SIZE) { setError(`${file.name} is larger than 5 MB.`); return; }
    setError("");
    const prev = form.photoPreviews[type];
  ...
  const moveToStep = (nextStep: 1 | 2 | 3 | 4) => {
    ...
    window.setTimeout(() => { setStep(nextStep); setTransitioning(false); }, 180);
  };
  ...
  const uploadDocument = async (file: File, label: string, startingProgress: number, progressRange: number) => {
    ...
    const result = await uploadFiles("documentUploader", {
      ...
      onUploadProgress: (progress) => {
        const value = startingProgress + (Number(progress) / 100) * progressRange;
        setUploadProgress(Math.min(99, value));
      },
    });
    return result[0];
  };
  ...
  const submitApplication = async () => {
    if (!validStep1 || !validStep2 || !validStep3 || !form.vehicleType) return;
    setUploading(true);
    setError("");
    setUploadProgress(2);
    ...
    try {
      ...
      const documentUrls: Record<FileType, string> = { license: "", registration: "", insurance: "", check: "", ssn: "" };

      for (let i = 0; i < documentEntries.length; i++) {
        const [type, file] = documentEntries[i];
        const doc = DOCUMENTS.find((d) => d.id === type);
        const uploaded = await uploadDocument(file, doc?.label ?? type, 5 + i * 14, 12);
        if (!uploaded?.url) throw new Error(`Upload failed for ${type}.`);
        documentUrls[type] = uploaded.url;
      }

      const photoEntries = Object.entries(form.photos) as [PhotoType, File][];
      const photoUrls: Record<PhotoType, string> = { front: "", driverSide: "", passengerSide: "", rear: "" };

      for (let i = 0; i < photoEntries.length; i++) {
        const [type, file] = photoEntries[i];
        setUploadStage(`Uploading ${PHOTOS.find((p) => p.id === type)?.label ?? type} photo…`);
        const uploaded = await uploadFiles("photoUploader", {
          files: [file],
          onUploadProgress: (progress) => {
            setUploadProgress(Math.min(98, 75 + i * 5 + (Number(progress) / 100) * 5));
          },
        });
        if (!uploaded?.[0]?.url) throw new Error(`Photo upload failed for ${type}.`);
      ...
      console.error(error);
      setError(error instanceof Error ? error.message : "Something went wrong while submitting.");
  ...
  if (showSuccess) {
    return (
      <main className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-6">
        <div className="w-full max-w-2xl rounded-3xl border border-white/10 bg-slate-900/80 backdrop-blur-xl p-8 sm:p-12 text-center shadow-2xl">
          ...
          <button onClick={() => router.push("/")} className="mt-8 w-full rounded-xl bg-gradient-to-r from-cyan-500 to-blue-500 px-6 py-4 text-lg font-bold shadow-lg shadow-cyan-500/20">
            Back to Home
          </button>
        </div>
      </main>
    ...
    <main className="min-h-screen overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-4xl flex-col px-4 py-8 sm:px-6">
        ...
        <div className="mb-8">
          <div className="mb-3 flex items-center justify-between">
            {stepLabels.map((label, index) => {
              ...
              return (
                <button key={label} type="button" disabled={uploading} onClick={() => { if (number <= step) moveToStep(number as 1 | 2 | 3 | 4); }} className="flex flex-col items-center gap-2">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 shadow-2xl backdrop-blur-xl sm:p-8">
              ...
              <div className="space-y-3">
                {DOCUMENTS.map((document) => {
                  ...
                  const inputRef = getInputRef(document.id);
                  return (
                    <div key={document.id} className={`relative flex w-full items-center justify-between rounded-2xl border p-5 text-left transition-all ${file ? "border-emerald-400/20 bg-emerald-400/5" : "border-white/10 bg-white/[0.03] hover:border-cyan-400/30"}`}>
                      ...
                      <input ref={inputRef} hidden type="file" accept=".pdf,image/jpeg,image/png,image/webp" onChange={(e) => { setDocument(document.id, e.currentTarget.files?.[0] ?? null); e.currentTarget.value = ""; }} />
                    </div>
                  );
                })}
              </div>
                ...
                {PHOTOS.map((photo) => {
                  ...
                  return (
                    <div key={photo.id} className="group relative aspect-video overflow-hidden rounded-2xl border border-white/10 bg-slate-900/70">
                      ...
                      <input ref={inputRef} hidden type="file" accept="image/*" onChange={(e) => { setPhoto(photo.id, e.currentTarget.files?.[0] ?? null); e.currentTarget.value = ""; }} />
                    </div>
                  );
                })}
              </div>
        ...
        <footer className="mt-8 flex gap-3">
          <button type="button" disabled={step === 1 || uploading} onClick={() => moveToStep((step - 1) as 1 | 2 | 3 | 4)} className="rounded-xl border border-white/10 bg-white/[0.04] px-6 py-3.5 font-bold text-slate-300 disabled:cursor-not-allowed disabled:opacity-40">
            Back
          </button>
          {step < 4 ? (
            <button type="button" disabled={uploading} onClick={handleContinue} className="flex-1 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-500 px-6 py-3.5 font-black shadow-lg shadow-cyan-500/20 disabled:opacity-50">
              Continue
            </button>
          ) : (
            <button type="button" disabled={uploading} onClick={submitApplication} className="flex-1 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 px-6 py-3.5 font-black shadow-lg shadow-emerald-500/20 disabled:cursor-not-allowed disabled:opacity-50">
